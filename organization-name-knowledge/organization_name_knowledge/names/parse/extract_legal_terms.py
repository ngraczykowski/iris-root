...
from organization_name_knowledge.names.parse.cut_terms import cut_terms, cut_until_any_term_matches
...
def extract_legal_terms(
    ...
    (
        without_legal_at_the_end,
        end_legal,
        end_other,
    ) = _extract_legal_from_chosen_name_part(name, legal_terms, from_start=False)

    (
        without_legal_at_start,
        start_legal,
        start_other,
    ) = _extract_legal_from_chosen_name_part(name, legal_terms, from_start=True)

    if start_legal.endswith(end_legal.cleaned_name) and start_legal != end_legal:
        if _check_is_legal_part_of_name(start_legal):
            return start_other + start_legal + without_legal_at_start, start_legal, TokensSequence()
        else:
...
def _check_is_legal_part_of_name(legal: TokensSequence) -> bool:
    return len(legal.cleaned_name.split()) < 3 or any(x in legal for x in ["of", "and", "for"])
