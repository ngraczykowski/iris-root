...
public class ReasoningBranchValidator {
  ...
  private static List<Long> toBranchIds(
      List<BranchIdAndSignatureDto> existingBranchIdsAndSignatures) {

    return existingBranchIdsAndSignatures
        .stream()
        .map(BranchIdAndSignatureDto::getBranchId)
        .collect(toList());
  }

  private static void validateNoDifferencesInIds(
  ...
  private static Map<Long, String> mapOf(List<BranchIdAndSignatureDto> existingBranches) {
    return existingBranches
        .stream()
        .collect(toMap(
            BranchIdAndSignatureDto::getBranchId,
            BranchIdAndSignatureDto::getFeatureVectorSignature));
  }

  private static void validateNoDifferencesInSignatures(
