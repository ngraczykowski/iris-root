...
import com.silenteight.sens.webapp.backend.user.registration.domain.*;
...
import static com.silenteight.sens.webapp.backend.user.registration.UserRegistrationUseCaseFixtures.*;
...
class RegisterInternalUserUseCaseTest {
  ...
  @BeforeEach
  void setUp() {
    UserRegistrationUseCaseConfiguration configuration = new UserRegistrationUseCaseConfiguration();
    ...
    underTest =
        configuration.registerInternalUserUseCase(userRegisteringService, registeredUserRepository);
  }
  ...
  void usernameTooLong_returnsValidDomainError() {
    ...
    assertThatResult(actual).isFailureOfType(InvalidNameLength.class);
  }
  ...
  class GivenUsernameIsNotUnique {
    ...
    @BeforeEach
    void setUp() {
      given(usernameUniquenessValidator.validate(any())).willReturn(Option.of(USERNAME_NOT_UNIQUE));
    }
  ...
  class GivenUsernameIsUniqueAndRolesDontExist {
    ...
    void setUp() {
      given(usernameUniquenessValidator.validate(any())).willReturn(Option.none());
      given(rolesValidator.validate(any())).willReturn(Option.of(ROLES_DONT_EXIST));
    }
