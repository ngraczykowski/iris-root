...
@Import({
    PendingBulkChangeRestController.class,
...
class PendingBulkChangeRestControllerTest extends BaseRestControllerTest {
  ...
  class ListBulkChangesIds {
    ...
    private final PendingBulkChangeRestControllerTest.ListBulkChangesIds.Fixtures
        fixtures = new PendingBulkChangeRestControllerTest.ListBulkChangesIds.Fixtures();

    @TestWithRole(roles = { BUSINESS_OPERATOR })
    void its200WithCorrectBody_whenFound() {
      given(bulkChangeQuery.getIdsOfPending(fixtures.reasoningBranchIds)).willReturn(
          List.of(
    ...
    private String mappingForIds(String reasoningBranchIds) {
      return format(
          "/bulk-changes/ids?reasoningBranchId=%s&statesFamily=pending", reasoningBranchIds);
    }
