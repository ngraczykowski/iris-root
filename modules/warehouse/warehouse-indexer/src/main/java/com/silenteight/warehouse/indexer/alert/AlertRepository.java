...
public interface AlertRepository {
   ...
   *
   * <p>e.g. of {@code filters} structure [ - filter_name: recommendation - values:FALSE_POSITIVE,
   * INVESTIGATE, - filter_name: riskType - values: PEP it tells us to fetch only alerts where
   * recommendation is FALSE_POSITIVE or INVESTIGATE and riskType is PEP
   *
   * @param alertColumnName name of the alertDateColumns which will be used as property name
   * @param timeFrom time from
   * @param timeTo time to
   * @param limit limit of fetched {@link AlertDto}
   * @param filters filter which will be used to filer out {@link AlertDto}
   * @param alertNames alert names
   * @return list of {@link AlertDto}
   */
  List<AlertDto> fetchRandomAlerts(
      AlertColumnName alertColumnName,
      String timeFrom,
      String timeTo,
      int limit,
   ...
   * @param timeFrom time from
   * @param timeTo time to
   * @param filters filter which will be used to filer out {@link AlertGroupingDto}
   * @param groupByFields list of fields to group by {@link AlertGroupingDto}
   */
  ...
  List<AlertDto> fetchAlertsByNames(List<String> alertNameList);

  List<AlertDto> fetchAlertsByNamesAndCountries(List<String> alertNameList, List<String> countries);
}
