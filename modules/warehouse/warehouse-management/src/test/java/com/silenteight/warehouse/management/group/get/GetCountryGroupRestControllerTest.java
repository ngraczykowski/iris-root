...
class GetCountryGroupRestControllerTest extends BaseRestControllerTest {
  ...
  private static final String GET_COUNTRY_GROUP_URL = COUNTRY_GROUP_URL + "/" + COUNTRY_GROUP_ID;
  ...
  void its200_whenCountryGroupFound() {
    when(countryGroupQuery.get(COUNTRY_GROUP_ID)).thenReturn(COUNTRY_GROUP_DTO);

    get(GET_COUNTRY_GROUP_URL)
        .statusCode(OK.value())
        .body("id", is(COUNTRY_GROUP_ID.toString()))
        .body("name", is(NAME));
  }
