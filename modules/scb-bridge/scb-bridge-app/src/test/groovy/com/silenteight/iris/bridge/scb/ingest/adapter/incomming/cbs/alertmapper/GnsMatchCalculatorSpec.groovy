...
class GnsMatchCalculatorSpec extends Specification {
  ...
  def 'calculate flags into #expectedResult for neoFlag: #neoFlag, batchId: #batchId'() {
    given:
    def suspect = new com.silenteight.iris.bridge.scb.ingest.adapter.incomming.common.hitdetails.model.Suspect(neoFlag: neoFlag, batchId: batchId)
    def lastDecBatchId = ''
  ...
  def 'calculate flags when neo flag is empty, #batchId, #lastDecBatchId, #hasLastDecision'() {
    given:
    def suspect = new com.silenteight.iris.bridge.scb.ingest.adapter.incomming.common.hitdetails.model.Suspect(batchId: batchId)
