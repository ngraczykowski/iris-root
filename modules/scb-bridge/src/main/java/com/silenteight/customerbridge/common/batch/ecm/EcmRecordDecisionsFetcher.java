...
import static com.silenteight.customerbridge.common.batch.ecm.EcmQueryTemplates.DECISIONS_QUERY;
import static com.silenteight.customerbridge.common.batch.ecm.ExternalId.HIT_UNIQUE_ID;
import static com.silenteight.customerbridge.common.batch.ecm.ExternalId.SYSTEM_ID;
import static com.silenteight.customerbridge.common.batch.ecm.ExternalId.tryToExtractWatchlistIdFromHitUniqueId;
  ...
  private String prepareFetchQuery() {
    if (log.isTraceEnabled()) {
      log.trace("Executing SQL: ecmViewName={}, query={}", ecmViewName,
          WHITESPACE.matcher(DECISIONS_QUERY).replaceAll(" "));
    }

    return DECISIONS_QUERY
