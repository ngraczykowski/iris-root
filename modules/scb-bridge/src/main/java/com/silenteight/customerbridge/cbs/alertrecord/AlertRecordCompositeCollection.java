...
import com.silenteight.customerbridge.cbs.alertrecord.InvalidAlert.Reason;
...
import static com.silenteight.customerbridge.cbs.alertrecord.InvalidAlert.Reason.ABSENT;
import static com.silenteight.customerbridge.cbs.alertrecord.InvalidAlert.Reason.WRONG_BATCH_ID;
...
public class AlertRecordCompositeCollection {
  ...
  AlertRecordCompositeCollection(
    ...
    for (AlertId alertId : alertIds) {
      ...
      if (alertRecord == null) {
        addInvalidAlert(systemId, batchId, ABSENT);
      } else if (!batchId.equals(alertRecord.getBatchId())) {
        addInvalidAlert(systemId, batchId, WRONG_BATCH_ID);
      } else {
