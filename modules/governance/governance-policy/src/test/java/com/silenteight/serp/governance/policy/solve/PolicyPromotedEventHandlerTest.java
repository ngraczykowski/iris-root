...
import static com.silenteight.serp.governance.policy.domain.dto.State.DRAFT;
...
class PolicyPromotedEventHandlerTest {
  ...
  void handlePolicyPromotedEvent() {
    ...
    PolicyPromotedEvent event = PolicyPromotedEvent.builder()
        .policyId(policyId)
        .build();
    when(policyService.getPolicy(policyId)).thenReturn(policy);
