...
@Slf4j
class UseAnalystSolutionSolvingStrategy implements SolvingStrategy {
  ...
  public SolveResponse solve(SolveRequest request) {
    ...
    if (request.isPreviousAnalystSolutionIn(
  ...
  private static boolean isPotentialTruePositiveOrHinted(RecommendedAction recommendedAction) {
    return recommendedAction == ACTION_POTENTIAL_TRUE_POSITIVE
