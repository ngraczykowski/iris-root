...
class AlertMessageStatusService implements AlertMessageStatusUseCase {
  ...
  @Override
  @Transactional
  public void transitionAlertDeliveryStatus(UUID alertMessageId, DeliveryStatus deliveryStatus) {
    var entity = repository.findByAlertMessageIdAndLockForWrite(alertMessageId).orElseThrow();

    entity.updateDeliveryStatus(deliveryStatus, clock);
    repository.save(entity);

    log.info("Alert [{}] transitioned to delivery status: {}", alertMessageId, deliveryStatus);
  }
  ...
  public void initState(UUID alertMessageId) {
    ...
    if (entity.isPresent()) {
      throw new IllegalStateException(
          "Unable to re-initialize the initial state for alertID: " + alertMessageId);
    }
