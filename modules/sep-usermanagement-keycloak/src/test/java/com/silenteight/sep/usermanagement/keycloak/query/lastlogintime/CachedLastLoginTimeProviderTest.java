...
import com.silenteight.sep.usermanagement.keycloak.query.lastlogintime.CachedLastLoginTimeProviderFixtures.LastLoginTime;
...
import static com.silenteight.sep.usermanagement.keycloak.query.lastlogintime.CachedLastLoginTimeProviderFixtures.USER_1_LAST_LOGIN_TIME;
...
class CachedLastLoginTimeProviderTest {
  ...
  void requiredNonCachedLastLoginTime_fetchesFromNestedProvider() {
    ...
    Optional<OffsetDateTime> actual = underTest.getForUserId(
        USER_1_LAST_LOGIN_TIME.getUserId());

    then(nestedLoginTimeProvider).should().getForUserId(
        USER_1_LAST_LOGIN_TIME.getUserId());
