...
class NnsEntitiesXmlNamesAndCountriesExtractor {
  ...
  private List<String> extractCountries() {
    ...
    var incorporationCountries =
        IncorporationCountryFeature.nnsEntitiesIncorporationCountries(matchData.getNnsEntities());

    var registrationCountries =
        RegistrationCountryFeature.worldCheckEntityCountries(matchData.getWorldCheckEntities());

    var otherCountries =
        new OtherCountryQueryFacade(matchData).mpWorldCheckEntitiesOtherCountries();
  ...
  private List<GetNameInformationResponseDto> extractNameInformation() {
    return matchData.getNnsEntities().stream()
        .flatMap(NnsEntitiesXmlNamesAndCountriesExtractor::extractWorldCheckListRecordIds)
        .filter(Objects::nonNull)
        .distinct()
        .map(
            listRecordId ->
  ...
  private static Stream<String> extractWorldCheckListRecordIds(
      NegativeNewsScreeningEntities worldCheckEntity) {
