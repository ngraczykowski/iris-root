...
public class AlertAgentDispatchProcess {
  ...
  private void solveReadyMatches(List<AlertSolving> pendingAlerts) {
    pendingAlerts.forEach(
        alertSolving -> Arrays.stream(alertSolving.getMatchIds()).forEach(matchId -> {
          if (alertSolving.isMatchReadyForSolving(matchId)) {
            var matchSolutionRequest = new MatchSolutionRequest(
                alertSolving.getAlertId(), matchId, alertSolving.getPolicy(),
