...
class RequestMissingFeatureValuesUseCase {
  ...
  private final MissingFeatureValueMatchesQueryRepository missingFeatureValueMatchesQueryRepository;
  ...
  private void process(List<MissingFeatureValueMatchesQueryEntity> list) {
    ...
    AgentExchangeEntity agentExchangeEntity;
    try {
      agentExchangeEntity = markAsRequested(list);
      ...
      if (e.getCause() instanceof ConstraintViolationException) {
        if (((ConstraintViolationException) e.getCause()).getConstraintName()
          ...
          log.info("Skipping executing agent requests for this chunk because one of the "
              + "Agent Request was submitted concurrently in a different transaction in the same "
              + "time: {}", e.getMessage());
          return;
        }
      }
    ...
    log.info(
        ...
        agentExchangeEntity.getId());
    sendAgentRequest(agentExchangeEntity);
  }

  @SneakyThrows
  @Transactional(propagation = Propagation.REQUIRES_NEW)
  private AgentExchangeEntity markAsRequested(
      String feature, String agentConfig, List<MissingFeatureValueMatchesQueryEntity> list) {
    var r = agentExchangeRepository.save(AgentExchangeEntity
        .builder()
        .feature()
        .matchFeatures(list.stream().map(e -> AgentExchangeMatchFeaturesEntity
            .builder()
            .matchId(e.getId()
                .getMatchId())
  ...
  public void sendAgentRequest(AgentExchangeEntity e) {
