CREATE OR REPLACE FUNCTION ae_select_unsolved_matches(in_analysis_id BIGINT)
  RETURNS TABLE
          (
            alert_id        BIGINT,
            match_id        BIGINT,
...
BEGIN
  ...
  CREATE TEMP TABLE tmp_ae_analysis_feature_vector_elements_query ON COMMIT DROP AS (
    SELECT *
  ...
  CREATE TEMP TABLE tmp_ae_analysis_match_feature_vector_elements_query ON COMMIT DROP AS (
    SELECT *
  ...
  CREATE TEMP TABLE tmp_ae_match_solution ON COMMIT DROP AS (
