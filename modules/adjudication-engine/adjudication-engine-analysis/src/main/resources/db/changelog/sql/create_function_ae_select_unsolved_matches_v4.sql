...
BEGIN
  ...
  CREATE TEMP TABLE tmp_ae_unsolved_match_ids ON COMMIT DROP AS (
    SELECT amms.alert_id
         , amms.match_id
    FROM tmp_ae_missing_match_solution amms
           LEFT JOIN tmp_analysis_match_agent_config_category_ids tamacci
           ...
           LEFT JOIN tmp_analysis_match_agent_config_feature_ids tamacfi
    ...
    WHERE COALESCE(ARRAY_LENGTH(_category_feature_ids.category_ids, 1), 0) =
          COALESCE(ARRAY_LENGTH(tamacci.category_ids, 1), 0)
      AND COALESCE(ARRAY_LENGTH(_category_feature_ids.feature_ids, 1), 0) =
          COALESCE(ARRAY_LENGTH(tamacfi.agent_config_feature_ids, 1), 0)
    LIMIT _limit
  );
  ...
  RETURN QUERY SELECT taumi.alert_id,
                      ...
                      COALESCE(aacaq.category_names, ARRAY []::varchar[]),
                      COALESCE(tca.category_values, ARRAY []::varchar[]),
                      COALESCE(aafaq.feature_names, ARRAY []::varchar[]),
                      COALESCE(tfa.feature_values, ARRAY []::varchar[]),
                      COALESCE(tfa.feature_reasons, ARRAY []::jsonb[]),
                      COALESCE(tfa.agent_configs, ARRAY []::varchar[])
               FROM tmp_ae_unsolved_match_ids taumi
                      JOIN ae_match am ON am.match_id = taumi.match_id
                      LEFT JOIN ae_analysis_categories_agg_query aacaq
                                ON aacaq.analysis_id = _analysis_id
                      LEFT JOIN ae_analysis_features_agg_query aafaq
                                ON aafaq.analysis_id = _analysis_id
                      LEFT JOIN tmp_category_aggregates tca
                                ON taumi.match_id = tca.match_id
                      LEFT JOIN tmp_feature_aggregates tfa
                                ON taumi.match_id = tfa.match_id
               WHERE COALESCE(ARRAY_LENGTH(aacaq.category_names, 1), 0) =
                     COALESCE(ARRAY_LENGTH(tca.category_values, 1), 0)
                 AND COALESCE(ARRAY_LENGTH(aafaq.feature_names, 1), 0) =
                     COALESCE(ARRAY_LENGTH(tfa.feature_values, 1), 0)
                 AND COALESCE(ARRAY_LENGTH(aafaq.feature_names, 1), 0) =
                     COALESCE(ARRAY_LENGTH(tfa.feature_reasons, 1), 0)
                 AND COALESCE(ARRAY_LENGTH(aafaq.feature_names, 1), 0) =
                     COALESCE(ARRAY_LENGTH(tfa.agent_configs, 1), 0);
END ;
$$;
