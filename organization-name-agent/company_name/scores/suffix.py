...
def _cast_to_common_base(tokens: TokensSequence) -> str:
    return " ".join(
        [
            KnowledgeBase.common_suffixes.source_to_common_base[t]
            for t in tokens.cleaned_tuple
        ]
    )


def get_suffix_fuzzy_score(
    ...
    return Score(
        value=fuzz.ratio(_cast_to_common_base(first), _cast_to_common_base(second))
        / 100,
