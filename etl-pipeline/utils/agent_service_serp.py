...
from silenteight.agent.document.v1.api.document_agent_pb2 import (
    CompareDocumentNumbersRequest, Facts, SolveDocNoRequest)
from silenteight.agent.document.v1.api.document_agent_pb2_grpc import \
    DocumentAgentStub
...
document_comparer_stub = factory.get_stub(
    DocumentAgentStub, "grpc-document-comparer-agent"
)
...
def call_name_agent(
    instance_name: str, alerted_names: List[str], matched_names: List[str]
):
...
def call_document_number_agent(
    alerted_documents: List[str], matched_documents: List[str]
):
...
def call_geo_location_agent(
    alerted_location: str, watchlist_location: str
