...
id_patterns = {"IBAN": r"\b[A-Z]{2}[0-9]{2}[A-Z0-9]{1,35}", "PESEL": r"\b[0-9]{11}"}


def parse_id_features(item):
    id_features = []
    if not item:
        return []
    for pat_name, pat in id_patterns.items():
        ...
        if found:
            id_features.append({pat_name: found.group()})
    return id_features


def print_id_features(id_features):
    for feature in id_features:
        print("tag name ", feature.child)
        for featured in feature.parsed_item:
            print(featured)
