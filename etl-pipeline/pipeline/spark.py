...
class Spark:
    ...
    def spark_read_csv(self, file_path):
        return (
            self.spark_instance.read.format("csv")
            .option("header", True)
    ...
    def write_and_get_delta_data(self, df, delta_path: str, user_metadata: str = ""):
        ...
        df.write.format("delta").option("overwriteSchema", "true").option("userMetadata", user_metadata).mode(
            "overwrite"
        ).save(delta_path)
    ...
    def group_count(
        ...
        if window_cols is None:
            ...
            sql_order_by_str = "window_sum desc, count desc, " + ", ".join(_backtick_elements(order_by_cols))
            ...
            df = (
                df.selectExpr("*", f"sum(count) over (partition by {sql_partition_by_str}) as window_sum")
                .selectExpr(
                    "*",
                    f"sum(count) over (order by {sql_order_by_str}) as count_cum_sum",
                    f"round(100 * sum(count) over (order by {sql_order_by_str}) \
    ...
    def sql_merge_to_target_col_from_source_cols(
        self, df, target_col, *source_cols, return_array=False, keep_duplicates=False
    ):
    ...
    def sql_cast_col_to_array(self, df, col):
        ...
        if isinstance(dtype, ArrayType):
            ...
            return F.when((F.isnull(col)) | (F.lower(col) == "none") | (F.col(col) == ""), F.array()).otherwise(
                F.array(col)
            )

    def to_pandas(self, spark_df, n=20):
        """Return as Pandas dataframe.

        :param spark_df: Spark dataframe passed in.
        :param n: The number of rows to display.
        :return: Pandas dataframe with specified top n rows.
        """
...
def set_spark_cpu_memory(spark_conf):
    ...
    spark_conf.setAll([("spark.master", "%s" % master), ("spark.driver.memory", "%s" % memory)])
