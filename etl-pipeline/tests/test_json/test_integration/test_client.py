...
class BaseGrpcTestCase:
    ...
    class TestGrpcServer(AsyncTestCase):
        ...
        @pytest.mark.asyncio
        async def test_isg_party_in_payload_format_customer_type_unknown(self):
            request_alert = load_alert(
            ...
            for match in request_alert.matches:
                compare_tested_uds_features_with_reference(
                    f'/tmp/features_{match.match_name.replace("/", "_")}.json',
                    f'tests/test_json/test_integration/expected_features/test_isg_party_in_payload_format_customer_type_unknown/features_{match.match_name.replace("/", "_")}.json',
                )

        @pytest.mark.asyncio
        async def test_isg_account_in_payload_format_customer_type_individual(self):
            request_alert = load_alert(
            ...
            for match in request_alert.matches:
                compare_tested_uds_features_with_reference(
                    f'/tmp/features_{match.match_name.replace("/", "_")}.json',
                    f'tests/test_json/test_integration/expected_features/test_isg_account_in_payload_format_customer_type_individual/features_{match.match_name.replace("/", "_")}.json',
                )
