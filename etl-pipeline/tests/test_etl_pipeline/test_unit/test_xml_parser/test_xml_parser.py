...
from customized.xml_pipeline import AIAWatchlistExtractor, AIAXMLPipeline
...
from tests.test_etl_pipeline.data.test_get_wl_hit_aliases_matched_name import (
...
TEST_PATH = "tests/test_etl_pipeline"
TEST_DATA_REFERENCE_DIR = "tests/data/"
...
class TestAlertHitDicttFactory(unittest.TestCase):
    ...
    def test__invoke_xml_util(self):
        input_xml = load_xml(f"{TEST_PATH}/data/test_input_extract_alert_and_hits_from_xml.xml")
        ...
        reference_json = load_json(
            f"{TEST_PATH}/data/test_output_extract_alert_and_hits_from_xml.json"
        )
    ...
    def test__create_spark_schema(self):
        ...
        reference_output = load_pickle(f"{TEST_PATH}/data/test_output_get_hit_spark_schema.pkl")
        self.assertEqual(result, reference_output)

    def test_get_alert_spark_schema(self):
        result = self.uut.get_alert_spark_schema()
        reference_output = load_pickle(f"{TEST_PATH}/data/test_output_get_alert_spark_schema.pkl")
        self.assertEqual(result, reference_output)
...
class TestAIAWatchlistExtractor(unittest.TestCase):
    ...
    def test_remove_unnecessary_columns(self):
        df = spark_instance.read_delta(
            f"{TEST_PATH}/data/test_input_remove_unnecessary_columns.delta"
        )
        result = self.uut.remove_unnecessary_columns(df)
        reference_df = spark_instance.read_delta(
            f"{TEST_PATH}/data/test_output_remove_unnecessary_columns.delta"
        )
        self.assertTrue(compare_dataframe(result, reference_df))

    def test_unwrap_alert_hits(self):
        pipeline = AIAXMLPipeline(spark_instance)
        df = spark_instance.read_delta(f"{TEST_PATH}/data/test_input_unwrap_alert_hits.delta")
        result = self.uut.unwrap_alert_hits(df, pipeline.schema)
        reference_df = spark_instance.read_delta(
            f"{TEST_PATH}/data/test_output_unwrap_alert_hits.delta"
        )
        self.assertTrue(compare_dataframe(result, reference_df))
...
class TestXMLPipeline(unittest.TestCase):
    def setUp(self):
        self.uut = AIAXMLPipeline(spark_instance)

    def test_unwrap_xml(self):
        result = self.uut.unwrap_xml(f"{TEST_DATA_REFERENCE_DIR}2.standardized/ALERTS.delta")
        df = spark_instance.read_delta(f"{TEST_PATH}/data/test_output_unwrap_xml.delta")
        self.assertTrue(compare_dataframe(result, df))

    def test_clear_empty_hits(self):
        df = spark_instance.read_delta(f"{TEST_PATH}/data/test_input_clear_empty_hits.delta")
        result = self.uut.clear_empty_hits(df)
        reference_df = spark_instance.read_delta(
            f"{TEST_PATH}/data/test_output_clear_empty_hits.delta"
        )
        self.assertTrue(compare_dataframe(result, reference_df))

    def test_pipeline(self):
        created_df = self.uut.pipeline(f"{TEST_DATA_REFERENCE_DIR}2.standardized/ALERTS.delta")
        df = spark_instance.read_delta(f"{TEST_PATH}/constant/output_xmlparser.delta")
